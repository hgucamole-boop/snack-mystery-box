...
import { snackItems, rarityColors, caseConfig, getRandomItemForCase } from '@/app/drop/data/snackItems';
import { useStore } from '@/store/useStore';

export function SpinReel({ onResult, triggerSpin, onSnackClick }) {
  ...
  const generateReelItems = (winner) => {
    const items = [];
    for (let i = 0; i < 40; i++) {
      if (i === 37) {
        items.push(winner);
