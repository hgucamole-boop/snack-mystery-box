...
const CategoryCard = ({ 
  ...
  return (
    <button
      type="button"
      onClick={onClick}
      className={`snack-card group relative block w-full text-left cursor-pointer ${bgColor} rounded-xl overflow-hidden transition-all hover:-translate-y-2 hover:shadow-xl ${sizeClass}`}
    >
      <img 
        alt={title} 
      ...
      <div className={`snack-card-content relative p-6 h-full flex flex-col ${sizeClass?.includes('large') || sizeClass?.includes('tall') ? 'justify-between' : 'justify-end'}`}>
        <div>
...
export function SnacksSection() {
  ...
  const modalContent = selectedCategory ? (
    ...
    >
      ...
      <div
        className="relative z-[10000] w-full max-w-3xl rounded-2xl bg-surface-container-high p-6 md:p-8 shadow-2xl border border-outline-variant animate-[fadeIn_180ms_ease-out]"
